...
def main(args=None):
    ...
    # META DATA
    pretrained_pth = 'path/to/tarined/model_state_dict.pt'
    output_root = 'output/giana_infer'
    # if not os.path.exists(output_root) : 
    #     os.mkdir(output_root)
    os.makedirs(output_root,exist_ok=True)
    image_pth = '/data1/whl/Datasets/xdecoder_data/GIANA/test'
    ...
    stuff_classes = ['polyp']
    # stuff_classes=CHOLECSEG8K_CLASSES
    ...
    with torch.no_grad():
        ...
        for file_name in os.listdir(image_folder):
            ...
            # sem_seg = (~(outputs[-1]['sem_seg'].max(0)[9])).squeeze().type(torch.uint8)
            # sem_seg = (outputs[-1]['sem_seg'].max(0)[1]==9).type(torch.uint8)
