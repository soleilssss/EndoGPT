...
def get_config_from_name(cfg, dataset_name):
    ...
    if 'refcoco' in dataset_name:
        ...
        return cfg
    elif 'sun_seg' in dataset_name and 'cls' not in dataset_name:
        cfg.update(cfg['SUN_SEG'])
        return cfg
    elif 'sun' in dataset_name and 'seg' not in dataset_name:
        cfg.update(cfg['SUN'])
        return cfg
        ...
        cfg.update(cfg['SAM'])
        return cfg
    elif 'mypolyp_sem_seg' in dataset_name:
        cfg.update(cfg['MYPOLYP_SEMSEG'])
        return cfg
    elif 'mypolyp_inst_seg' in dataset_name:
        cfg.update(cfg['MYPOLYP_INSTSEG'])
        return cfg
    elif 'giana' in dataset_name:
        cfg.update(cfg['GIANA'])
        return cfg
    elif 'piccolo' in dataset_name:
        cfg.update(cfg['PICCOLO'])
        return cfg
    elif 'polyp_all' in dataset_name:
        cfg.update(cfg['POLYP_ALL'])
        return cfg
    elif 'cholecseg8k' in dataset_name:
        cfg.update(cfg['CHOLECSEG8K'])
        return cfg
    elif 'colon_lap' in dataset_name:
        cfg.update(cfg['COLON_LAP'])
        return cfg
    elif 'polyp_cls_2' in dataset_name:
        cfg.update(cfg['POLYP_CLS_2'])
        return cfg
    elif 'polyp_cls' in dataset_name:
        cfg.update(cfg['POLYP_CLS'])
        return cfg
    elif 'fetreg' in dataset_name:
        cfg.update(cfg['FETREG'])
        return cfg
    elif 'sinus' in dataset_name:
        cfg.update(cfg['SINUS'])
        return cfg
    elif 'm2caiseg' in dataset_name:
        cfg.update(cfg['M2CAISEG'])
        return cfg
    elif 'fewshot_2' in dataset_name:
        cfg.update(cfg['FEWSHOT_2'])
        return cfg
    elif 'fewshot' in dataset_name:
        cfg.update(cfg['FEWSHOT'])
        return cfg
    elif 'sun_seg_cls' in dataset_name:
        cfg.update(cfg['SUN_SEG_CLS'])
        return cfg
    elif 'ldpolypvideo' in dataset_name:
        cfg.update(cfg['LDPOLYPVIDEO'])
        return cfg
    elif 'polypset' in dataset_name:
        cfg.update(cfg['POLYPSET'])
        return cfg
    elif '20250912' in dataset_name:
        cfg.update(cfg['C20250912'])
        return cfg
    else:
...
def build_eval_dataloader(cfg, ):
    ...
    for dataset_name in cfg['DATASETS']['TEST']:
        ...
        elif ('sun' in dataset_name) and ('sun_seg' not in dataset_name):
            mapper = SunRGBDSegDatasetMapper(cfg, False)
        elif 'refcoco' in dataset_name:
            mapper = RefCOCODatasetMapper(cfg, False)
        elif 'polyp_inst_seg' in dataset_name:
            mapper = PolypInstDatasetMapper(cfg, False)
        else:
            mapper = None
...
def build_train_dataloader(cfg, ):
    ...
    for idx,dataset_name in enumerate(dataset_names):
        ...
        elif mapper_name == "mask_former_semantic_multiclass":
            mapper=MaskFormerSemanticDatasetMapperMultiClass(cfg,True)
            # loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
            if idx==0:
                loaders['coco'] = build_detection_train_loader(cfg, dataset_name=dataset_name, mapper=mapper)
            else:
...
def build_evaluator(cfg, dataset_name, output_folder=None):
    ...
    # POLYP_INSTANCE_SEG:
    # if evaluator_type == "polyp_inst_seg" and cfg_model_decoder_test["INSTANCE_ON"]:
    #     evaluator_list.append(InstanceSegEvaluator(dataset_name, output_dir=output_folder))
